...
const PlaneJS = new Player(100, 200); //      PLANE W JS

class Move {
    static RenderPlane(PlaneJS) {
        const PlaneDom = document.getElementById('player'); //    PLANE w DOM
        PlaneDom.style.left = PlaneJS.x + "px"; //zostawic left lub right
    ...
    static KeySupport(PlaneJS, event) {
        switch (event.code) {
            ...
            case "ArrowDown":
                ...
                break;
            default:
                Move.RenderPlane(PlaneJS); //NIE WIADOMO CO ROBI DEFAULT
        }
    }
    ...
    static Collision(PlaneJS) {
        if (
            PlaneJS.x === 0 || //
            PlaneJS.x === 880 || //offset
            PlaneJS.y === 0 ||
            PlaneJS.y === 580 //offset
        ) {
